-- Prescription engine: coverage metadata, idempotency, templates, events, configurable rules.
...
-- Traceability events (immutable append-only)
...
-- Reusable medication combinations (never auto-issue)
...
-- Per-clinic configurable coverage rules (JSON; no normativa hardcoded)
