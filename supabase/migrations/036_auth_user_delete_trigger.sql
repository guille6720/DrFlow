-- Permite eliminar usuarios desde Supabase Dashboard (Authentication → Users).
-- Antes del DELETE en auth.users, limpia referencias en public.profiles.
...
CREATE OR REPLACE FUNCTION handle_auth_user_before_delete()
RETURNS TRIGGER
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public, auth
AS $$
BEGIN
  PERFORM cleanup_user_profile_references(OLD.id, NULL);
  RETURN OLD;
END;
$$;
...
CREATE TRIGGER on_auth_user_before_delete
  ...
  FOR EACH ROW
  EXECUTE FUNCTION handle_auth_user_before_delete();
...
-- Helper para SQL Editor (Dashboard): borrar por email
...
REVOKE ALL ON FUNCTION handle_auth_user_before_delete() FROM PUBLIC;
REVOKE ALL ON FUNCTION delete_auth_user_by_email(TEXT) FROM PUBLIC;
