...
function WhatsAppIcon() {
  return (
    <svg
      className="drflow-ehr-print-whatsapp-icon"
      viewBox="0 0 24 24"
      aria-hidden="true"
      focusable="false"
    >
...
export function PatientEhrPrintDemographics({ patient }: Props) {
  ...
  const insuranceLabel = patient.insurance_provider?.toUpperCase().includes("PAMI")
    ? "PAMI"
    : patient.insurance_provider?.trim() || "Obra social";
    ...
    <section className="drflow-ehr-print-demographics" aria-label="Datos del paciente">
      <div className="drflow-ehr-print-demo-grid">
        ...
        <div className="drflow-ehr-print-demo-field">
          ...
          <div className="drflow-ehr-print-demo-value drflow-ehr-print-demo-age">
            {birthDate ? <p className="drflow-ehr-print-demo-birth">{birthDate}</p> : null}
            <p>{detailedAge ?? "Sin definir"}</p>
          </div>
        </div>
        <div className="drflow-ehr-print-demo-field">
          <p className="drflow-ehr-print-demo-label">{insuranceLabel}</p>
          {patient.insurance_number ? (
            <p className="drflow-ehr-print-demo-value"># {patient.insurance_number}</p>
          ) : (
            <p className="drflow-ehr-print-demo-value">Sin definir</p>
          )}
        </div>
        <div className="drflow-ehr-print-demo-field">
          <p className="drflow-ehr-print-demo-label">Teléfono</p>
          <p className="drflow-ehr-print-demo-value drflow-ehr-print-demo-phone">
            {phone ? (
              <>
                <WhatsAppIcon />
                <span>{phone}</span>
