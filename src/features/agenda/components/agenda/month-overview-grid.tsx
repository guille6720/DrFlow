...
const MonthDayCell = memo(function MonthDayCell({
  ...
  return (
    ...
    >
      ...
      {count > 0 ? (
        <span className="mt-auto text-xs font-bold leading-tight text-teal-900">
    ...
    <div className="drflow-card-light overflow-hidden rounded-2xl bg-white text-slate-900 ring-1 ring-slate-200">
      ...
      <div className="grid grid-cols-7 border-b border-slate-200 bg-slate-100/80">
        {WEEK_LABELS.map((label) => (
          <div
            key={label}
            className="px-1 py-2.5 text-center text-xs font-bold uppercase tracking-wide text-slate-700"
          >
            {label}
          </div>
        ))}
