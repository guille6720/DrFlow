...
export type CreateTurnoWizardResult =
...
async function insertStaffAppointmentDirect(
  ...
  userId: string,
  data: TurnoWizardInput
): Promise<CreateTurnoWizardResult> {
...
export async function executeCreateTurnoWizard(input: unknown): Promise<CreateTurnoWizardResult> {
  try {
    ...
    if (error) {
      if (shouldTryDirectInsertFallback(error)) {
        ...
        return { ok: false, error: resolveCreateTurnoRpcError(error) };
      }
    } else {
    ...
    try {
      ...
      logServerError("createTurnoWizard.audit", auditErr, { clinicId, persist: false });
    }

    try {
      revalidateAppointmentSurfaces({ patientId: data.patient_id });
    } catch (revalidateErr) {
      logServerError("createTurnoWizard.revalidate", revalidateErr, { clinicId, persist: false });
    }
